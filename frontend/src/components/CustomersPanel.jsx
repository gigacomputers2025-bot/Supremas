          ...
          <tbody>
            {customers.map((c, idx) => (
              <tr key={c.id}>
                ...
                <td>
                  <InlineEdit value={[c.calle, c.altura, c.piso_dto].filter(Boolean).join(' ')} onSave={v => {
                    const parts = v.split(' ');
                    save(c.id, 'calle', parts[0] || '');
                    save(c.id, 'altura', parts.slice(1).join(' '));
                  }} />
                </td>
                <td>
                  <InlineEdit value={c.barrio} onSave={v => save(c.id, 'barrio', v)} />
                </td>
                <td>
...
function InlineEdit({ value, onSave }) {
  ...
  return (
    <span onClick={() => setEditing(true)} style={{ cursor: 'pointer', minHeight: 22, display: 'block', padding: '2px 0' }} title="Click para editar">
