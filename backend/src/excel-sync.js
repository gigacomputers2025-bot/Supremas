  ...
  if (wsProducts) {
    ...
    const transaction = db.transaction((rows) => {
      for (const row of rows) {
        const result = insertProduct.run(row.listType, row.category, row.name, row.sigla, row.isCombo ? 1 : 0, row.sortOrder);
    ...
    // Import categories from products' category column (deduplicated)
    const insertCat = db.prepare(`INSERT OR IGNORE INTO categories (name) VALUES (?)`);
    const catDone = new Set();
    for (const row of productRows) {
      if (row.category && !catDone.has(row.category.toLowerCase())) {
        catDone.add(row.category.toLowerCase());
        insertCat.run(row.category);
      }
    }
