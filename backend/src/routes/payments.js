...
router.post('/', (req, res) => {
  ...
  auditLog('CREATE', 'payment_methods', result.lastInsertRowid, null, { name, commission_rate }, req.ip);
  res.status(201).json(pm);
});

router.put('/:id', (req, res) => {
  const db = getDb();
  const oldData = db.prepare(`SELECT * FROM payment_methods WHERE id = ?`).get(req.params.id);
  const { name, commission_rate } = req.body;
  db.prepare(`UPDATE payment_methods SET name = ?, commission_rate = ? WHERE id = ?`).run(name, commission_rate, req.params.id);
  auditLog('UPDATE', 'payment_methods', Number(req.params.id), oldData, { name, commission_rate }, req.ip);
...
router.delete('/:id', (req, res) => {
  ...
  auditLog('DELETE', 'payment_methods', Number(req.params.id), oldData, null, req.ip);
