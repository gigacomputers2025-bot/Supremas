...
router.get('/', (req, res) => {
  const db = getDb();
  const { search, barrio, canal, fecha_desde, fecha_hasta, limit = 100, offset = 0 } = req.query;
...
router.post('/', (req, res) => {
  ...
  auditLog('CREATE', 'orders', result.lastInsertRowid, null, { cliente, total: total_pedido }, req.ip);
  res.status(201).json(db.prepare(`SELECT * FROM orders WHERE id = ?`).get(result.lastInsertRowid));
});

router.put('/:id', (req, res) => {
  const db = getDb();
  const oldData = db.prepare(`SELECT * FROM orders WHERE id = ?`).get(req.params.id);
  const {
    conductor, mes, anio, fecha_pedido, fecha_entrega, dia,
    canal_venta, cliente, dni, celular, calle, altura, piso_dto,
    comentario, barrio, lista_precio,
  ...
  auditLog('UPDATE', 'orders', Number(req.params.id), oldData, { cliente, total: total_pedido }, req.ip);
...
router.delete('/:id', (req, res) => {
  ...
  auditLog('DELETE', 'orders', Number(req.params.id), oldData, null, req.ip);
